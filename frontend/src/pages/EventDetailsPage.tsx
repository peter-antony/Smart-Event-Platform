        ...
        <div className="p-6 lg:p-8 grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="space-y-2">
              <h2 className="text-lg font-bold text-slate-900 dark:text-white">Event Description</h2>
            ...
            <div className="glass-card p-5 rounded-2xl border border-slate-200 dark:border-gray-800 space-y-3">
              <h3 className="text-sm font-bold text-slate-900 dark:text-white flex items-center gap-2">
                <Calendar className="w-4 h-4 text-brand-500 dark:text-brand-400" /> Date & Time Schedule
              </h3>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs text-slate-700 dark:text-gray-300">
                <div>
                  <span className="text-slate-500 dark:text-gray-400 block text-[11px]">Event Date</span>
                  <span className="font-bold text-slate-900 dark:text-white text-sm">{formatDate(event.start_time)}</span>
                </div>
                <div>
                  <span className="text-slate-500 dark:text-gray-400 block text-[11px]">Start & End Time</span>
            ...
            <div className="glass-card p-5 rounded-2xl border border-slate-200 dark:border-gray-800 space-y-3">
              <h3 className="text-sm font-bold text-slate-900 dark:text-white flex items-center gap-2">
                <Building className="w-4 h-4 text-brand-500 dark:text-brand-400" /> Location & Venue Details
              </h3>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs text-slate-700 dark:text-gray-300">
                <div>
                  <span className="text-slate-500 dark:text-gray-400 block text-[11px]">Venue Name</span>
                  <span className="font-bold text-slate-900 dark:text-white text-sm">{venueNameStr}</span>
                </div>
                <div>
                  <span className="text-slate-500 dark:text-gray-400 block text-[11px]">Full Address</span>
                  <span className="font-bold text-slate-900 dark:text-white text-sm flex items-center gap-1">
            ...
            <div className="space-y-3">
              <h3 className="text-sm font-bold text-slate-900 dark:text-white">Event Features & Pass Benefits</h3>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-2 text-xs text-slate-700 dark:text-gray-300">
                <div className="flex items-center gap-2 bg-slate-100 dark:bg-gray-900/60 p-3 rounded-xl border border-slate-200 dark:border-gray-800">
                  <CheckCircle2 className="w-4 h-4 text-emerald-600 dark:text-emerald-400 shrink-0" /> Full access to live keynotes & sessions
                </div>
                <div className="flex items-center gap-2 bg-slate-100 dark:bg-gray-900/60 p-3 rounded-xl border border-slate-200 dark:border-gray-800">
                  <CheckCircle2 className="w-4 h-4 text-emerald-600 dark:text-emerald-400 shrink-0" /> Interactive Q&A and networking lounge
                </div>
                <div className="flex items-center gap-2 bg-slate-100 dark:bg-gray-900/60 p-3 rounded-xl border border-slate-200 dark:border-gray-800">
                  <CheckCircle2 className="w-4 h-4 text-emerald-600 dark:text-emerald-400 shrink-0" /> Digital certificate of attendance
                </div>
                <div className="flex items-center gap-2 bg-slate-100 dark:bg-gray-900/60 p-3 rounded-xl border border-slate-200 dark:border-gray-800">
                  <CheckCircle2 className="w-4 h-4 text-emerald-600 dark:text-emerald-400 shrink-0" /> On-demand video session recordings
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="space-y-4">
            <div className="glass-card p-6 rounded-2xl border border-brand-500/30 space-y-5 sticky top-24 shadow-glow">
              ...
              <div className="flex items-center justify-between pb-4 border-b border-slate-200 dark:border-gray-800">
                <div>
                  <span className="text-xs text-slate-500 dark:text-gray-400">Price per ticket</span>
                  <div className="text-3xl font-extrabold text-slate-900 dark:text-white">${event.price.toFixed(2)}</div>
                </div>
              ...
              <div className="space-y-2">
                <div className="flex items-center justify-between text-xs text-slate-700 dark:text-gray-300">
                  <label className="font-bold text-slate-900 dark:text-white">Select Ticket Quantity</label>
                  <span className="text-slate-500 dark:text-gray-400 text-[11px]">Max: {maxTicketQuantity}</span>
                </div>

                <div className="flex items-center justify-between p-2.5 rounded-2xl bg-slate-100 dark:bg-gray-900 border border-slate-200 dark:border-gray-800">
                  {/* Minus Button */}
                  <button
                    type="button"
                    disabled={ticketsCount <= 1 || isBookingDisabled}
                    onClick={handleDecrement}
                    className="w-9 h-9 rounded-xl bg-white dark:bg-gray-800 hover:bg-slate-200 dark:hover:bg-gray-700 disabled:opacity-40 disabled:cursor-not-allowed font-bold text-slate-900 dark:text-white flex items-center justify-center transition-colors shadow-sm"
                  >
                    <Minus className="w-4 h-4" />
                  </button>

                  <span className="text-base font-extrabold text-slate-900 dark:text-white px-3 font-mono">{ticketsCount}</span>
              ...
              <div className="p-3.5 rounded-2xl bg-brand-500/10 dark:bg-brand-950/30 border border-brand-500/20 flex items-center justify-between">
                <span className="text-xs font-semibold text-slate-700 dark:text-gray-300">Total Amount</span>
                <span className="text-xl font-extrabold text-brand-600 dark:text-brand-400">${totalPrice.toFixed(2)}</span>
              </div>

              {/* Book Now Button */}
              {isBookingDisabled ? (
                <div className="p-3 rounded-2xl bg-red-500/10 dark:bg-red-950/40 border border-red-500/30 text-center space-y-1">
                  ...
                  <p className="text-[11px] text-slate-500 dark:text-gray-400">Booking is not available for this event status.</p>
                </div>
              ) : (
                <Button
                  variant="primary"
              ...
              <div className="flex items-center justify-center gap-2 text-[11px] text-slate-500 dark:text-gray-400 pt-1">
                <ShieldCheck className="w-3.5 h-3.5 text-emerald-600 dark:text-emerald-400" /> Instant E-Ticket & Pass Confirmation
              </div>
            </div>
          </div>
        </div>
