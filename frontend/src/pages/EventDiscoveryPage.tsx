    ...
    <div className="space-y-8 animate-in fade-in duration-200">
      ...
      <div className="relative overflow-hidden rounded-2xl glass-panel p-8 lg:p-10 border border-brand-500/30 bg-gradient-to-r from-brand-900/10 via-indigo-900/10 to-purple-900/10 dark:from-brand-950/80 dark:via-indigo-950/60 dark:to-purple-950/50 transition-colors">
        <div className="relative z-10 max-w-2xl space-y-4">
          ...
          <h1 className="text-3xl lg:text-5xl font-extrabold tracking-tight text-slate-900 dark:text-white leading-tight font-sans">
            Discover & Experience <span className="bg-gradient-to-r from-brand-600 via-indigo-600 to-purple-600 dark:from-brand-400 dark:via-indigo-300 dark:to-purple-400 bg-clip-text text-transparent">Smart Events</span>
          </h1>
          <p className="text-slate-600 dark:text-gray-300 text-sm lg:text-base leading-relaxed">
      ...
      {notifyToast && (
        <div className="glass-panel p-4 rounded-2xl border border-brand-500/40 bg-brand-500/10 dark:bg-brand-950/50 flex items-center gap-3 text-xs text-brand-800 dark:text-brand-200 animate-in fade-in zoom-in duration-200">
          <CheckCircle2 className="w-5 h-5 text-brand-500 dark:text-brand-400 shrink-0" />
      ...
      <div className="space-y-4">
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-xl font-bold text-slate-900 dark:text-white tracking-tight">Published Events</h2>
          </div>
          <span className="text-xs text-slate-500 dark:text-gray-400 font-medium">
            Showing <strong className="text-slate-900 dark:text-white">{filteredAndSortedEvents.length}</strong> published event(s)
          </span>
        </div>

        {/* Filter Toolbar */}
        <div className="glass-card p-4 rounded-2xl border border-slate-200 dark:border-gray-800 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3">
          ...
          <div className="relative">
            ...
            <input
              ...
              className="w-full bg-white dark:bg-gray-900 border border-slate-200 dark:border-gray-800 rounded-xl pl-9 pr-3 py-2 text-xs text-slate-900 dark:text-white placeholder-gray-400 dark:placeholder-gray-500 focus:outline-none focus:border-brand-500"
            />
          </div>
          ...
          <select
            ...
            className="bg-white dark:bg-gray-900 border border-slate-200 dark:border-gray-800 rounded-xl px-3 py-2 text-xs text-slate-900 dark:text-white focus:outline-none focus:border-brand-500"
          >
            {categories.map((cat) => (
          ...
          <button
            ...
            className="bg-white dark:bg-gray-900 border border-slate-200 dark:border-gray-800 hover:border-brand-500 rounded-xl px-3 py-2 text-xs text-slate-700 dark:text-gray-300 flex items-center justify-between transition-colors"
          >
            <span className="flex items-center gap-1.5">
              <ArrowUpDown className="w-3.5 h-3.5 text-brand-500 dark:text-brand-400" />
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {[1, 2, 3].map((n) => (
            <div key={n} className="glass-card h-96 rounded-2xl animate-pulse p-6 bg-slate-100 dark:bg-gray-900/40 border border-slate-200 dark:border-gray-800 space-y-4">
              ...
              <div className="h-10 bg-slate-200 dark:bg-gray-800/60 rounded-xl w-full mt-4" />
            </div>
          ))}
        </div>
      ) : error ? (
            ...
            return (
              ...
              >
                ...
                <div className="p-5 space-y-4 flex-1 flex flex-col justify-between">
                  <div className="space-y-2">
                    <h3 className="text-base font-extrabold text-slate-900 dark:text-white group-hover:text-brand-600 dark:group-hover:text-brand-300 transition-colors line-clamp-2">
                      {evt.title}
                    </h3>
                  ...
                  {/* Metadata Row */}
                  <div className="space-y-2 pt-2 border-t border-slate-200 dark:border-gray-800/80 text-xs text-slate-700 dark:text-gray-300">
                    <div className="flex items-center gap-2">
                    ...
                    <div className="flex items-center justify-between text-[11px] pt-1">
                      <div className="flex items-center gap-1.5 text-slate-500 dark:text-gray-400">
                        <Ticket className="w-3.5 h-3.5 text-brand-500 dark:text-brand-400" />
                        <span>Available Seats:</span>
                        <strong className="text-slate-900 dark:text-white font-bold">{seatsAvailable}</strong>
                      </div>

                      {evt.is_virtual && (
                        <span className="px-2 py-0.5 rounded-md bg-purple-500/20 text-purple-700 dark:text-purple-300 font-bold border border-purple-500/30">
                          Virtual
                        </span>
                      )}
                    </div>
