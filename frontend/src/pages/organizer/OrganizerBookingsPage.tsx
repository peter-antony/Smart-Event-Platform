      ...
      <div className="glass-panel p-6 rounded-3xl border border-purple-500/30 bg-gradient-to-r from-purple-950/50 via-indigo-950/40 to-brand-950/50 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          <div className="flex items-center gap-2">
            <h1 className="text-xl font-extrabold text-white tracking-tight">Attendee Reservations</h1>
          ...
          <p className="text-xs text-gray-400">Track attendee ticket passes, reference codes, and reservation payouts</p>
        </div>
      </div>
      ...
      <div className="glass-panel rounded-2xl border border-gray-800 overflow-hidden">
        <div className="overflow-x-auto">
          <table className="w-full text-left text-xs">
            <thead className="bg-gray-900/80 border-b border-gray-800 text-gray-400 uppercase text-[10px] font-bold tracking-wider">
              <tr>
            ...
            <tbody className="divide-y divide-gray-800/60 text-gray-200">
              {filteredBookings.map((b) => (
                <tr key={b.id} className="hover:bg-purple-950/20 transition-colors">
                  <td className="p-4 font-mono font-bold text-purple-300">{b.booking_reference}</td>
                  <td className="p-4 font-bold text-white max-w-xs truncate">{b.event_title}</td>
                  <td className="p-4">
                  ...
                  <td className="p-4 font-bold text-emerald-400">${b.total_amount.toFixed(2)}</td>
                  <td className="p-4">
                    <span className="px-2 py-1 rounded-full text-[10px] font-bold bg-emerald-500/20 text-emerald-300 border border-emerald-500/30">
                      {b.status}
                    </span>
                  </td>
                </tr>
