...
export const OrganizerCreateEventPage: React.FC<OrganizerCreateEventPageProps> = ({ onNavigate }) => {
  // Form State
  ...
  const handleSaveDraft = async (e: React.MouseEvent) => {
    ...
    try {
      ...
      setToastMessage({
        type: 'draft',
        text: `Draft saved! Event "${createdEvent.title || title}" has been stored as a DRAFT.`
      });
  ...
  const handlePublishEvent = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setToastMessage({
        type: 'success',
        text: `🎉 Success! Event "${createdEvent.title || title}" has been published to live attendee inventory.`
      });
