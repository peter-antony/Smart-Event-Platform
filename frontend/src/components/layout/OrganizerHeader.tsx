    ...
    <header className="sticky top-0 z-30 w-full glass-panel border-b border-slate-200 dark:border-gray-800/80 px-4 lg:px-8 py-3.5 flex items-center justify-between transition-colors">
      <div className="flex items-center gap-4">
        ...
        <button
          ...
          className="lg:hidden p-2 rounded-xl text-slate-500 dark:text-gray-400 hover:text-slate-900 dark:hover:text-white hover:bg-slate-100 dark:hover:bg-gray-800/60 transition-colors"
          ...
          <div>
            <div className="flex items-center gap-2">
              <span className="text-xl font-extrabold tracking-tight text-slate-900 dark:text-white font-sans">
                Event<span className="text-purple-600 dark:text-purple-400">ora</span>
              </span>
            </div>
          </div>
        </div>
      ...
      <div className="flex items-center gap-3.5">
        ...
        <div className="relative">
          <button
            ...
            className="p-2 rounded-xl text-slate-500 dark:text-gray-400 hover:text-slate-900 dark:hover:text-white hover:bg-slate-100 dark:hover:bg-gray-800/60 transition-colors relative"
            title="Organizer Notifications"
          >
            <Bell className="w-5 h-5" />
            <span className="w-2.5 h-2.5 rounded-full bg-purple-500 absolute top-1.5 right-1.5 ring-2 ring-white dark:ring-gray-950 animate-pulse"></span>
          </button>
            ...
            <div className="absolute right-0 mt-2 w-80 glass-panel rounded-2xl p-4 border border-purple-500/30 shadow-2xl space-y-3 z-50 animate-in fade-in zoom-in duration-150">
              <div className="flex items-center justify-between pb-2 border-b border-slate-200 dark:border-gray-800">
                <span className="text-xs font-bold text-slate-900 dark:text-white flex items-center gap-1.5">
                  <Bell className="w-3.5 h-3.5 text-purple-600 dark:text-purple-400" /> Notifications
                </span>
                <button onClick={() => setShowNotificationMenu(false)} className="text-slate-400 dark:text-gray-400 hover:text-slate-900 dark:hover:text-white">
                  <X className="w-4 h-4" />
                </button>
              </div>
              ...
              <div className="space-y-2 max-h-64 overflow-y-auto pr-1">
                {mockNotifications.map((n) => (
                  <div key={n.id} className={`p-2.5 rounded-xl text-xs space-y-1 ${n.unread ? 'bg-purple-50 dark:bg-purple-950/40 border border-purple-200 dark:border-purple-500/20' : 'bg-slate-100 dark:bg-gray-900/40 border border-slate-200 dark:border-gray-800'}`}>
                    <div className="flex items-center justify-between">
                      <span className="font-bold text-slate-900 dark:text-white text-[11px]">{n.title}</span>
                      <span className="text-[9px] text-slate-400 dark:text-gray-400">{n.time}</span>
                    </div>
                    <p className="text-[11px] text-slate-600 dark:text-gray-300 leading-tight">{n.message}</p>
                  </div>
                ))}
              </div>
              ...
              <button
                ...
                className="w-full text-center text-[11px] font-bold text-purple-600 dark:text-purple-400 hover:text-purple-500 dark:hover:text-purple-300 pt-1 block"
              >
