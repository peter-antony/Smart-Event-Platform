  ...
  const getNotificationIcon = (type: string) => {
    switch (type) {
      case 'BOOKING_CONFIRMED':
        return <div className="p-2 rounded-xl bg-emerald-500/20 text-emerald-400 border border-emerald-500/30"><Ticket className="w-4 h-4" /></div>;
      case 'EVENT_REMINDER':
        return <div className="p-2 rounded-xl bg-purple-500/20 text-purple-400 border border-purple-500/30"><Clock className="w-4 h-4" /></div>;
      case 'EVENT_UPDATED':
      ...
      case 'EVENT_CANCELLED':
        return <div className="p-2 rounded-xl bg-red-500/20 text-red-400 border border-red-500/30"><AlertTriangle className="w-4 h-4" /></div>;
      case 'CALENDAR_ADDED':
        return <div className="p-2 rounded-xl bg-indigo-500/20 text-indigo-400 border border-indigo-500/30"><Calendar className="w-4 h-4" /></div>;
      default:
        return <div className="p-2 rounded-xl bg-gray-800 text-gray-400"><Bell className="w-4 h-4" /></div>;
    }
  };
  ...
  return (
    <div className="relative" ref={dropdownRef}>
      ...
      <button
        ...
        className="relative p-2.5 rounded-2xl glass-card border border-gray-800 hover:border-gray-700 text-gray-300 hover:text-white transition-all shadow-sm flex items-center justify-center"
        ...
        <div className="absolute right-0 mt-3 w-80 sm:w-96 glass-panel rounded-3xl p-4 shadow-2xl border border-purple-500/30 z-50 animate-in fade-in zoom-in-95 duration-150">
          ...
          <div className="flex items-center justify-between pb-3 border-b border-gray-800">
            <div className="flex items-center gap-2">
              <h3 className="font-extrabold text-sm text-white tracking-tight">In-App Notifications</h3>
              {unreadCount > 0 && (
                ...
                <div
                  ...
                  className={`p-3 rounded-2xl border transition-all cursor-pointer flex gap-3 ${
                    n.is_read
                      ...
                      : 'bg-gradient-to-r from-purple-950/40 to-gray-900 border-purple-500/30 shadow-sm hover:border-purple-500/50'
                  }`}
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-start justify-between gap-1">
                      <h4 className="text-xs font-bold text-gray-200 truncate">{n.title}</h4>
                      {!n.is_read && (
